...
const distanceBetweenObj = (obj1, obj2) => Math.sqrt(Math.pow(obj1.x - obj2.x, 2) + Math.pow(obj1.y - obj2.y, 2));
...
const tick = () => {
    if (ctx) {
        ...
        rocks.forEach((rock) => {
            ...
            rock.y += rock.yv;
            clampToScreen(rock);
            const collideWithBullet = bullets.find((bullet) => distanceBetweenObj(rock, bullet) < rockSize[rock.size]);
            if (collideWithBullet) {
                rock.size -= 1;
                const speed = 2;
                rock.xv = -speed + Math.random() * speed * 2;
                rock.yv = -speed + Math.random() * speed * 2;
                const rockB = Object.assign(Object.assign({}, rock), { xv: -speed + Math.random() * speed * 2, yv: -speed + Math.random() * speed * 2 });
                rocks.push(rockB);
                collideWithBullet.timeToLive = 0;
            }
            const collideWithPlayer = player.timeToRespawn === 0 &&
